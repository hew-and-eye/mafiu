...
export function getParsedTemplate(template: string, state: Record<string, any>): string {
  ...
  template.match(attrReferenceRegex)?.forEach(ref => {
    ...
    const replacement = `has-mdeps mdep-${attr}-${variableName} ${attr}="${state[variableName] ?? ''}"` // Results in stuff like '<img mdep-src-imageSource src="{{imageSource}}" />
  ...
  template.match(contentReferenceRegex)?.forEach(ref => {
    ...
    const replacement = `<span has-mdeps mdep--${variableName}>${state[variableName] ?? ""}</span>` // Results in stuff like '<div>Look, a value: <span mdep--stateVariable>{{stateVariable}}</span>
    parsedTemplate = parsedTemplate.replace(ref, replacement)
  });
